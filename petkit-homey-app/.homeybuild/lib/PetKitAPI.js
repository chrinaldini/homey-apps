...
class PetKitAPI {
  ...
  // HTTP helper
  // -------------------------------------------------------------------
  _request(method, path, { body = null, formEncoded = false, extraHeaders = {} } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const headers = {
        ...
        'User-Agent':    'PETKIT/8.2.1 (iPhone; iOS 16.0; Scale/3.00)',
        'X-Timezone':    String(REGION_TIMEZONE[this.region] ?? 1.0),
      ...
      const req = lib.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (parsed.error) {
              reject(new Error(parsed.error.msg || JSON.stringify(parsed.error)));
            } else {
              resolve(parsed);
            }
          } catch (e) {
  ...
  async login() {
    ...
    const timezone    = REGION_TIMEZONE[this.region] ?? 1.0;
  ...
  async getDevices() {
    ...
    for (const d of roster.feeders       || []) devices.push(this._normalizeDevice(d, 'feeder'));
    for (const d of roster.devices       || roster.litterBoxes || []) devices.push(this._normalizeDevice(d, this._guessCategory(d.type)));
    for (const d of roster.waterFountains|| []) devices.push(this._normalizeDevice(d, 'fountain'));
    for (const d of roster.purifiers     || []) devices.push(this._normalizeDevice(d, 'purifier'));

    if (devices.length === 0 && Array.isArray(result.result)) {
      for (const d of result.result) devices.push(this._normalizeDevice(d, this._guessCategory(d.type)));
    }
  ...
  // Litter box commands
  // -------------------------------------------------------------------
  async startCleaning(deviceId) { return this._litterCommand(deviceId, 'start_clean'); }
  async stopCleaning(deviceId)  { return this._litterCommand(deviceId, 'stop_clean'); }
  async odorRemoval(deviceId)   { return this._litterCommand(deviceId, 'odor_removal'); }
  async dumpLitter(deviceId)    { return this._litterCommand(deviceId, 'dump_litter'); }

  async _litterCommand(deviceId, cmd) {
    return this._request('POST', '/cat/litter/clean', { body: { deviceId, cmd } });
  }
  ...
  // Static helpers
  // -------------------------------------------------------------------
  static getRegions()            { return Object.keys(REGION_URLS).filter(r => r !== 'default'); }
  static getDeviceTypeName(type) { return DEVICE_TYPE_MAP[type] || `Unknown (${type})`; }
}
